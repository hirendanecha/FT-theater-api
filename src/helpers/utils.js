...
exports.channelNotificationEmail = async (userData) => {
  let name = userData?.userName;
  let msg = `You have been assign in Fuck Theater channel by the Fuck Theater Admin.
  To access your channel, log into your Fuck Theater account,click on the
  Fuck Theater icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `Fuck Theater has approved your Connection application.`;
    } else {
      msg = `Fuck Theater has unapproved your Connection application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
